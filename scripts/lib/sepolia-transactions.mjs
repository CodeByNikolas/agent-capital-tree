...
export async function journaledTransaction({ rpc, signer, directory, name, request, confirmations = 2, maxGasCostWei = parseEther('0.025') }) {
  if (!/^[a-zA-Z0-9_-]{1,80}$/.test(name)) throw new Error('Invalid journal name');
  if (typeof maxGasCostWei !== 'bigint' || maxGasCostWei <= 0n || maxGasCostWei > parseEther('0.025')) throw new Error('Invalid transaction gas cap');
  ...
  try {
    ...
    if (!fees.maxFeePerGas || gasLimit * fees.maxFeePerGas > maxGasCostWei) throw new Error('Transaction fee ceiling exceeded');
  ...
  if (!transaction.maxFeePerGas || transaction.gasLimit * transaction.maxFeePerGas > maxGasCostWei) throw new Error('Transaction fee ceiling exceeded');

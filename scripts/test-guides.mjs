...
import { tourStepHref, tourExitHref } from '../apps/web/src/lib/tour-steps.ts';

const base = process.env.ACT_TEST_APP_URL ?? 'http://localhost:3044';
for (let step = 1; step <= 5; step++) {
  const url = new URL(tourStepHref(step, { vault: 'my-root.agentcapitalvault.eth' }), base);
  assert.equal(url.searchParams.get('vault'), 'my-root.agentcapitalvault.eth');
  assert.equal(url.searchParams.has('root'), false);
  assert.equal(new URL(tourExitHref(step, { vault: 'my-root.agentcapitalvault.eth' }), base).searchParams.has('tour'), false);
}
...
try {
  for (const width of [1280, 380]) {
    ...
    for (let step = 1; step <= 5; step++) {
      ...
      await expect(guide).toContainText(`Step ${step} of 5`);
      ...
      if (step < 5) await guide.getByRole('link', { name: 'Next', exact: true }).click();
  ...
  console.log('PASS: onboarding hide/reopen, reachable MCP instructions, all five tour steps, preserved vault/preview context, desktop/mobile and no browser errors. No wallet actions.');

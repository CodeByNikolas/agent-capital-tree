...
import { JsonRpcProvider, Contract, Interface, toBeHex, zeroPadValue } from 'ethers';
...
try{
  ...
  const checks=[];
  for(const nodeId of [report.childId,report.grandchildId]){
    const node=await controller.getNode(nodeId,{blockTag:head.number});
    await assert.rejects(controller.checkAction(nodeId,1n<<44n,node.agent,0,1,{blockTag:head.number}),
      error=>error.code==='CALL_EXCEPTION'&&error.data===iface.getError('Inactive').selector);
    checks.push({nodeId,operation:'checkAction(SWAP, recorded agent, token0, amount1)',result:'Inactive',method:'eth_call'});
  }
  ...
  if(process.argv.includes('--write')){
    ...
    report.directAuthorizationChecks={chainId:11155111,blockNumber:head.number,checks};

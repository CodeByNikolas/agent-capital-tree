...
- Neue Chat-Einrichtung: `scripts/mcp-readonly-server.mjs` ist ein separater STDIO-MCP mit nur `getTree`, `pnpm mcp:chat-verify` testet ihn. `codex mcp add capital_tree_readonly -- <node-exe> <absolute-script-path>` schreibt eine **persönliche** MCP-Registrierung; der temporäre `mcp:verify`-Lauf tut das nicht. Die ChatGPT-Desktop-App teilt Codex-Konfiguration; normaler ChatGPT-Webchat liest lokale STDIO-Konfiguration nicht. Keine globale Nutzerkonfiguration ohne Auftrag ändern. Dashboard darf lokalen MCP-Status nicht aus Vault-Daten vortäuschen. Bereits vorhandenen Checkout nutzen, keinen zweiten Clone im Projekt erzeugen.

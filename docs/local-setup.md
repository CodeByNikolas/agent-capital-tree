...
The answer should name Sepolia chain `11155111`, root 3 and a recent block. Always read the current funding and rights from that snapshot: `hello` changed externally from empty to 0.10 Test-USDC with active rights during development. Do not assume an old balance or reuse it for a write test without checking its owner/operator. You can also pass a vault address or numeric root ID. The dashboard cannot inspect the local STDIO session.

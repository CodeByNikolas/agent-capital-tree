...
7. **MCP live proof:** open [the public MCP guide](https://agent-capital-tree-silk.vercel.app/mcp) without selecting a vault. On a Judge laptop, use an existing `work/rami` checkout (do not clone again inside it), run the keyless checks, then register `capital_tree_readonly` and call `getTree` from a new Codex chat in the ChatGPT desktop app. `pnpm mcp:verify` separately proves temporary full-plugin installation and 16-tool discovery; the real chat server exposes only one read tool. Compare its Sepolia block, root vault and two nodes with the dashboard. No financial write is exercised or implied.

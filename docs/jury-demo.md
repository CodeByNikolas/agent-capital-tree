...
7. **MCP live proof:** open [the public MCP guide](https://agent-capital-tree-silk.vercel.app/mcp) without selecting a vault. In a Codex chat, call `getTree` with `hello.agentcapitalusdc.eth`: show its JSON-backed PNG graph, Sepolia block, zero Test-USDC and currently no authorized actions. The keyless server also offers `prepareRootSetup`, which generates a bounded browser-wallet link but does not sign. For an agent-write demo, set up a new small root, fund at most 0.10 Test-USDC, bind a separate local operator, start the WSL2 companion with writes enabled, and use its typed `spawnChild`/revoke/reclaim tools. Do not claim this public E2E succeeded without receipts. `pnpm mcp:verify` independently proves 16-tool discovery with writes disabled.

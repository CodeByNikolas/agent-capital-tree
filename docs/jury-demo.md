...
7. **MCP live proof:** open [the public MCP guide](https://agent-capital-tree-silk.vercel.app/mcp) without selecting a vault. On a Judge laptop, use an existing `work/rami` checkout (do not clone again inside it), run the keyless checks, then register `capital_tree_readonly` and call `getTree` and `visualizeTree` from a new Codex chat in the ChatGPT desktop app. `pnpm mcp:verify` separately proves temporary full-plugin installation and 16-tool discovery; the real chat server exposes only these two read tools. Show the PNG tree, its Sepolia block, root vault and two nodes. `pnpm mcp:desktop` can display a separate foreground connection status. No financial write is exercised or implied.

...
Workers run with Docker `--network none`. In native Codex mode, a host app-server holds a dedicated normal Codex login and sends execution to the worker exec-server through a private loopback relay. Scoped financial tools are forwarded by the host; account credentials and wallet keys stay outside the container. In explicit CLIProxyAPI mode, the existing worker-bound Unix gateway supplies scoped MCP and inference access. Container loopback is not host loopback. General outbound network access is blocked.

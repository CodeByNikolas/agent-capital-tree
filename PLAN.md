...
Neueste Nutzerentscheidung: Einmalige Owner-Wallet-Autorisierung, danach begrenzte Aktionen per Chat mit automatisch vorbereitetem lokalem Agent-Schlüssel. Der Schlüssel ist der Contract-Operator, nicht ein zweites Nutzerkonto; die Owner-Wallet wird niemals exportiert. Der Demo-Standard ist **Kapitalmodus**: vorhandener Companion ohne InferenceBroker, Docker-Launcher oder Modellkonfiguration. `createChildVault` erzeugt ENS/Child-Vault und Kapitalallokation ohne autonomen Modellprozess. Die Intelligenz kommt aus dem laufenden Codex-/Claude-Chat. `spawnChild` bleibt der explizite optionale Docker-/CLIProxyAPI-Workerpfad. Kein neuer Cloud-Signer und keine generische Transaktions-API.

`pnpm mcp:capital` übernimmt Linux-/WSL-Start, privates Profil, vorhandene Schlüssel und Companion-Lifecycle; keine manuelle Companion-JSON-Datei oder Tokenkopie. `getCapitalSetup` zeigt alle Voraussetzungen gemeinsam aus einem Block (USDC und natives ETH-Gas getrennt). `prepareCapitalSetup` darf neue Schlüssel nur für noch ungebundene Roots erzeugen und öffnet den normalen Wallet-Browser. Bereits gebundene Operatoren werden nicht automatisch ersetzt; bestehende Worker-/Gas-Journale verhindern einen ungeprüften Moduswechsel. Ein Child-Vault wird nie als laufender KI-Prozess ausgegeben. Der normale Companion-Katalog hat jetzt 17 Tools, der Kapital-MCP zusätzlich vier zusätzliche Read-/Setup-Tools (21). Einmaliger MCP-Neustart bei Wechsel des installierten Servers bleibt nötig; danach sind alle Werkzeuge auch vor Wallet-Setup sichtbar. Diese Entscheidung ersetzt untenstehende pauschale Docker-/CLIProxyAPI-Pflichten für Finanzaktionen; sie gelten nur für autonome Hintergrund-Worker.

Ergänzung: **Jeder** MCP-Toolaufruf erhält eine Grafik im Dashboard-Design, einschließlich Wallet-Setup, Aktionsresultaten, Validierungsfehlern und nicht bestätigten Ergebnissen. PNGs werden mit gebündeltem WASM und Dashboard-Schriften lokal erzeugt; kein externer Bilddienst. Bilder plus existierende lokale Markdown-Bildlinks und Mermaid-Fallback begleiten die Originaldaten. Große Trees werden in lesbare Seiten aus demselben Snapshot aufgeteilt. Keine neue Chain-Abfrage oder Write-Wiederholung allein für die Darstellung. Der Chat-Host muss die Darstellung unterstützen; keine universelle GUI-Garantie aus einem MCP-Result ableiten. `prepareRootSetup` öffnet direkt den normalen Standardbrowser mit bestehendem Wallet-Profil, signiert aber nichts.

Die untenstehende plattformneutrale L0–L8-Architektur ist für den ETHGlobal-Demoablauf zurückgestellt. Der MCP soll im normalen Codex-Chat ENS-Namen/Vault-Adressen auflösen und bei **jedem** Tree-Aufruf JSON, Bild und Mermaid aus einem blockkonsistenten Sepolia-Snapshot liefern. `hello.agentcapitalusdc.eth` (Root 3) ist zunächst ausschließlich ein Read-Beispiel. Öffentliche Finanzaktionen nur auf einem neu geprüften, kleinen Testroot; höchstens 0,10 Test-USDC an den Demo-Child. Keine alten Seed-/Payment-Transaktionen wiederholen.

Der Chat kann ein typisiertes Root-Setup vorbereiten und einen direkten Browser-Wallet-Link ausgeben. Die Browser-Wallet signiert die separaten Owner-Transaktionen für Root, exakte Tokenfreigabe/Finanzierung und Operator-Bindung; ein Wallet direkt **im Chat** bleibt Future Work. Danach signiert ausschließlich der lokal gebundene Root-Operator die typisierten Companion-Aktionen für Child-Erstellung, Delegation, Einschränkung/Widerruf und Rückholung, ohne erneute Owner-Signatur je Child. Keine generischen Transaktionstools oder Cloud-Signer. Chain 11155111 wird geprüft; Schlüssel, Provider-Credentials und MCP-Bearer bleiben außerhalb von Chat/Repo/Ausgaben. Windows-Write-Demo nutzt WSL2 und vorhandenen Docker-Companion, nicht einen neuen plattformneutralen Container-Umbau. Bei fehlender WSL2-/Docker-/Wallet-Freigabe keine E2E-Behauptung.
...
Der frühere Future-Work-Punkt zur Chat-Visualisierung ist für den Codex-CLI-Chat umgesetzt: der MCP liefert JSON, PNG und Mermaid aus einem Block. Das Rendering in der ChatGPT-Desktop-GUI und Claude Desktop bleibt separat zu prüfen; aus einer Visualisierung folgt keine zusätzliche Finanzberechtigung.

Nachfrage 26.09.: Für tatsächliche Codex-/Claude-Chats auf dem Laptop gibt es einen **separaten** schlüssellosen STDIO-Server mit `getTree`, `visualizeTree` und `prepareRootSetup`. Die ersten beiden liefern einen echten Sepolia-Snapshot als PNG mit Mermaid-Fallback; der dritte gibt nur einen Browser-Wallet-Link aus und signiert nichts. Ein optionaler lokaler Vordergrund-Monitor hält eine eigene MCP-Testsitzung offen; Codex und Claude starten weiterhin ihre eigenen STDIO-Prozesse. Der Server nutzt öffentliches Sepolia-RPC und darf niemals die 11 Write-Tools als funktionsfähig ausgeben. Der 16-Tool-Installationsnachweis bleibt ein temporärer Pakettest. Das Dashboard visualisiert den Pfad Codex → lokaler MCP → Sepolia und seinen eigenen unabhängigen Sepolia-Read; ohne explizites Pairing kann es keine lokale Codex-STDIO-Verbindung beobachten. Kein falscher „Connected“-Badge, keine öffentliche Heartbeat- oder Token-Übertragung. ChatGPT-Webchat ist nicht der lokale Codex-Desktop-Host.

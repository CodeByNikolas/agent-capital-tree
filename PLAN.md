...
## Current decisions — 27 September 2026

This section supersedes earlier deployment, UI and onboarding targets. [STATUS.md](STATUS.md) records actual work; [ACCEPTANCE.md](ACCEPTANCE.md) distinguishes current demo proof from historical model-worker tests.

### Namespace and publication

Use ENSv2 Sepolia `kanoki.eth` exclusively for new roots and the current dashboard/capital MCP. The cutover is complete; earlier prototypes remain on-chain but are excluded from current lookup and are not migrated. The mainnet name is outside this registration claim. Current addresses live in `deployments/usdc-sepolia.json`.

Work on `main`, preserve concurrent changes, commit as CodeByNikolas and push tested changes. The canonical app is https://kanoki-app.vercel.app, Vercel project `agent-capital-tree` in `tumblockchains-projects`. Deployment access is available here; use [the release checklist](docs/release-checklist.md). Keep the frontend build SHA distinct from later documentation-only commits.

### Current public demo and UI

The existing `capital.kanoki.eth` demo contains five real vaults over three levels, two LP NFTs, a confirmed x402 payment and MultiBaas-indexed allocations, swap, LP and policy events. See the public receipt reports in ACCEPTANCE.md. These recorded actions do not imply currently running model workers. Completed financial runners must not be replayed during a read-only presentation.

Preserve Kanoki branding, Fraunces/IBM Plex and explicit dark/light themes. Use the dark shadcn sidebar, lighter workspace and shared App Router layout; navigation preserves sidebar/header state and the header stays sticky. Primary actions are light green; Overview action cards are equally wide with bottom-aligned buttons. Wallet address opens Sign out/explorer controls; sign-out survives reload until explicit Connect. Remove the normal network badge, retaining an actionable wrong-network switch. How it works belongs on Overview; onboarding offers root creation and Open live demo. Setup integration status stays expanded. Use Skeletons for asynchronous data without showing stale/sample state, retain in-progress forms during refresh, and display ERC-20 amounts to three decimals (positive dust <0.001) without rounding inputs or transactions.

### Capital setup and optional autonomous workers

The default standalone plugin exposes 19 tools and starts without a root argument, runtime URL or bearer token. `prepareRootSetup` persists a private local signer/name and opens one guided wallet page. Up to five owner confirmations create the root, authorize the operator, approve/fund a shared 0.10 Test-USDC demo budget and fund up to 0.01 Sepolia ETH operator gas. Funding uses the aggregate tree shortfall and receipt journals prevent duplicate or ambiguous retries. After authorization, the matching root is discovered and restored across restarts. Advanced root selection/recovery keeps explicit write-target confirmation and controller/root checks.

`createChildVault` performs a real allocation and reports `dispatchStatus: not_requested`; it does not start a model. It needs no Docker or inference credentials. The three-tool keyless reader is a separate mode. The configured 17-tool worker bridge supports autonomous `spawnChild`, indexed history and paid-service configuration; these four unavailable tools are omitted from the default capital catalog. All modes retain graphics and structured data.

Autonomous workers require Docker, an explicitly configured inference path, a task, allocation, model and narrower mandate. The jury setup documents native API-key inference with Luna High as its preferred path and dedicated ChatGPT login as an alternative; the HomeBox operator uses its separately governed CLIProxyAPI configuration. Historical native API-key financial evidence is in ACCEPTANCE.md. Owner keys never enter the companion; each transaction signer needs its own Sepolia gas. No automatic Codex subagent hook creates on-chain custody.
...
Keep nine addressable pages: Overview, Agent Tree, Activity, Uniswap, x402 Pay, Curvegrid, Applications, MCP, and Setup. Use a shadcn/ui sidebar, adding Badge, Button, Card, Sheet, and Table as needed. Frontend Design, Impeccable, and shadcn MCP support the work. Acceptance requires a calm layout, 16px body text, at least 14px supporting text, ample spacing, and explicit light/dark mode.
...
Capital-mode jury prerequisites: wallet, Sepolia ETH/Test-USDC, Node.js and a supported Codex or Claude host. Flow: install plugin → ask for setup → approve the guided wallet steps → return to chat → create bounded child vaults → inspect the dashboard. Docker and separate inference configuration are required only for optional autonomous workers. Owner recovery remains a separately reviewed wallet action. The website needs no localhost access; the plugin client needs no public server. The shared MultiBaas instance covers public roots from its documented indexing start; disclose that boundary. Independent contract deployments need their own indexer configuration. Inspecting the demo and manual wallet actions work without model access.

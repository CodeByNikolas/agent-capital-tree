...
export function AgentActivity({ data, node, feed, loading, error, onSelect, onRetry, children }: {
  ...
  return <>
    <div className="page-heading"><span className="page-kicker">Curvegrid MultiBaas · Sepolia</span><h1>Agent activity</h1><p>Follow the capital assigned to an agent, its delegations, and its swaps through indexed on-chain events.</p></div>
      ...
      {loading ? <div role="status" aria-label="Loading agent activity"><Skeleton className="h-24 w-full" /><span>Loading MultiBaas events…</span></div>
        ...
        : <>
          ...
          <dl className="agent-report-totals">{totals.map(total => {
            ...
            return <div key={`${total.kind}:${total.token}`}><dt>{flowLabels[total.kind]}</dt><dd>{token ? `${formatAmount({ ...token, rawAmount: total.rawAmount.toString() })} ${token.symbol}` : `${total.rawAmount} raw units (${total.token})`}</dd></div>;
          })}</dl>

...
**Recommended demo:** [capital mode](../../docs/local-setup.md#recommended-one-time-kanoki-wallet-setup) automatically starts the existing companion and needs no Docker, CLIProxyAPI or hand-written configuration. The worker instructions below apply only to autonomous background inference. See the capital-mode section at the end of this file.
...
Historical native OpenAI API-key financial E2E passed on the retired pre-Kanoki Sepolia controller with real `gpt-6-luna` / `high` inference: [evidence](../../deployments/jury-openai-native.json). ChatGPT-login financial E2E remains open. The deterministic `test:native-protocol` check uses the real pinned CLI and Docker with a local Responses fixture: shell execution, file patches, host-file isolation, scoped finance forwarding and expiry pass without inference credentials. Its test-only wrapper substitutes fixture provider metadata; it does not verify normal account authentication. Set `ACT_CODEX_BINARY` and `ACT_WORKER_IMAGE_ID` to run it. Add `ACT_TEST_API_KEY=1` to exercise ephemeral API-key authentication with a synthetic key and assert that no auth file or container credential is created. Both are simulated inference checks. After building, an operator with an authenticated worker profile may run the optional real inference smoke:

```sh
ACT_NATIVE_CONFIG=/absolute/private/config.json node packages/runtime/test/native-live.integration.mjs
```

It makes a real Codex inference using synthetic task data, a read-only `getPaymentServices` call and a workspace proof; it makes no chain or financial writes. This smoke passed with a real OpenAI API key and Luna High; the separate financial run above establishes same-host API-key acceptance. ChatGPT login has not been tested live. The existing `model.integration.mjs` command is a HomeBox CLIProxyAPI-only acceptance check; it requires that optional config and its owner-only `0600` `providerTokenFile` and is restricted to the HomeBox endpoint.
...
For native inference, `codexBinary` is the absolute path to the same verified Linux ELF used by `build-worker-image.mjs`, rather than a shell wrapper. **Preferred:** set `openaiApiKeyFile` to an absolute, owned, nonsymlink regular file with mode `0600`, containing only the raw OpenAI API key. The host uses ephemeral app-server API-key authentication; no login or auth.json is required. Invalid explicit files fail without fallback. API usage is billed separately from vault capital. `codexHome` remains a dedicated owned `0700` directory. **ChatGPT alternative:** omit `openaiApiKeyFile` and authenticate this directory with `CODEX_HOME=/absolute/private/codex-home /absolute/path/to/linux-codex -c 'cli_auth_credentials_store="file"' login`; check it with `CODEX_HOME=/absolute/private/codex-home /absolute/path/to/linux-codex login status`. This worker login profile must remain pristine, without manual configuration, MCP registrations, apps or plugins. The pinned CLI’s generated `/workspace` trust entry and bundled `.system` skills are allowed. The root interactive CLI uses a separate profile with the root MCP registration; use its `/model` picker to copy an exact account-available name into `models`. Run `node packages/runtime/cli.mjs check-codex /absolute/private-config.json` before `prepare-root`: it checks authentication, configured model metadata (through the official OpenAI models API for API-key mode) and Docker without running inference or making financial calls; it does not establish inference quota or billing credit. The companion checks native availability again before child allocation. Authentication stays with the host app-server. The root MCP bearer is separate from Codex authentication.

For **optional HomeBox CLIProxyAPI inference**, set `"inference": "cliproxyapi"`, remove `codexBinary`, `codexHome` and `openaiApiKeyFile`, and add `"upstream": "http://your-cliproxyapi-host:8317/v1"` plus `"providerTokenFile": "/absolute/private/cliproxyapi-token"`. The upstream must be CLIProxyAPI; the worker never receives its credential. A present token file must be an absolute, owner-only `0600` regular file; invalid values fail closed. On HomeBox only, omitting the property uses the host-only `/usr/local/bin/codexops-proxy-token` helper. Start with `node packages/runtime/cli.mjs start /absolute/private-config.json`. It prints the local tools origin and private root MCP token path, and disables Sepolia writes by default. An explicit `--enable-sepolia-writes` flag is required for onchain worker actions. The CLI never prints the bearer.
...
`childGasWei` is zero by default. A nonzero base grant is capped at `0.025 Sepolia ETH` per root-created child; a grandchild receives one quarter of that base. The ETH comes from the parent operator and is sent once after the child allocation is confirmed. The companion writes the exact signed raw transaction and hash to a private `0600` journal before broadcast; a retry may only rebroadcast that same transaction. The gas fee ceiling is 20 gwei. The operator must size the grant against measured spawn gas and current fees; a configured grant does not guarantee another spawn is affordable. A missing or uncertain gas receipt prevents worker dispatch. There is no faucet or automatic refill. The Anvil integration uses disposable accounts. Later public financial runs are documented in [ACCEPTANCE.md](../../ACCEPTANCE.md); historical worker receipts must not be attributed to the current Kanoki demo.
# Capital-only demo mode

Use `node packages/runtime/capital.mjs settings --enable-sepolia-writes` after building SDK, MultiBaas, plugin and runtime. No companion JSON, Docker or CLIProxyAPI is needed for chat-managed vaults. The capital launcher supports native macOS, Linux and Windows→WSL2, discovers/reuses a matching private root profile and starts/stops the existing companion with the MCP. The default 19-tool plugin starts unbound: `prepareRootSetup` creates/reuses its private signer and opens guided wallet setup; the matching root is then discovered and restored automatically. An explicit ENS argument is an advanced selection option. Read the complete [capital setup](../../docs/local-setup.md).

`getCapitalSetup` reports local key, onchain binding, actual rights, Test-USDC balance/limits and **native ETH** gas from one block. `prepareCapitalSetup` prepares a key only for an unbound root and opens normal-browser owner authorization. `createChildVault` performs the existing atomic ENS/vault allocation without worker dispatch, credentials or a child gas grant. It returns `not_requested`, never “worker started”. The operation fingerprint includes execution mode; switching modes under the same idempotency key fails. Existing worker/gas state is not silently abandoned/rebroadcast in capital mode.

Owner and agent are distinct signing roles. Wallet connection/funding is not unattended transaction permission. The owner authorizes the local key once; the chat never receives the owner key. Autonomous workers below remain optional and need their own configured inference provider.

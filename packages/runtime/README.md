...
- `workerDockerArgs` checks real mount paths under `<runtimeRoot>/workers/<workerId>/` and constructs fixed Docker arguments. The trusted companion selects the host UID/GID that owns the private `0700` worker directory and workspace plus the disposable `0400`/`0600` decrypted key file. It must create and verify an **internal** `act-workers` Docker network with access only to the broker and required RPC; the network name alone provides no isolation. The image is pinned by SHA-256 digest. Feed the task on stdin; never insert it into Docker args or env. Start Docker using an argument array, with no shell.

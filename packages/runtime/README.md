...
**Recommended demo:** [capital mode](../../docs/local-setup.md#recommended-capital-demo-no-docker-or-cliproxyapi) automatically starts the existing companion and needs no Docker, CLIProxyAPI or hand-written configuration. The worker instructions below apply only to autonomous background inference. See the capital-mode section at the end of this file.
...
Use `node packages/runtime/capital.mjs settings <ENS-name> --enable-sepolia-writes` after building SDK, MultiBaas, plugin and runtime. No companion JSON, Docker or CLIProxyAPI is needed for chat-managed vaults. The launcher supports Linux and Windows→WSL2, discovers/reuses a matching private root profile and starts/stops the existing companion with the MCP. Read the complete [capital setup](../../docs/local-setup.md).

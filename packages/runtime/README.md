...
Indexed `getCapitalActivity` is optional. To enable it, add `"multibaas": { "deploymentUrl": "https://your-deployment.multibaas.com/", "controllerLabel": "capital-controller", "apiKeyFile": "/absolute/private/multibaas-key" }` to the private config. The key file must belong to the current user and have mode `0600`. The companion binds activity requests to the worker's root and its configured controller, and the MultiBaas adapter verifies indexed values and finality against Sepolia RPC receipts. Without this config the tool returns unavailable; no synthetic history is returned.

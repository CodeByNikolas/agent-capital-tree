...
if (command === 'settings') {
  ...
  if (command === 'check') console.log(serialize(await session.inspect()));
    ...
    const execute = async (name, input) => {
        ...
        if (!specs[name].readOnly) await session.preflight(target, name === 'createChildVault' ? args.operationKey : undefined);
        ...
        return { ...result, activeMcpRootId: session.rootId, targetRootId: session.rootId, backgroundWorker: 'not_requested' };
      };

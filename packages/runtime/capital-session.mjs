...
export class CapitalSession {
  constructor({ client, controller, base, repo, query, explicitRoot, writesEnabled, closeRuntime = async () => {} }) {
    Object.assign(this, { client, controller, base, repo, query, explicitRoot, writesEnabled, closeRuntime });
  }
  ...
  async inspect(budgetRaw = '100000', tree) {
    ...
    return { ...setup, activeMcpRootId: this.rootId, writesEnabled: this.writesEnabled,
  ...
  async tree(query) {
    ...
    return { ...resolved.tree, selectedNodeId: resolved.selectedNodeId,
      mcp: setup ?? { activeMcpRootId: this.rootId, viewedRootId: String(resolved.tree.rootId),
        targetMatches: false, writeReady: false, localOperator: null, operatorGasWei: null,
  ...
  async preflight(expectedRootId, operationKey) {
    ...
    if (!setup.checks.operatorHasGas) {
      ...
      const generation = operationKey ? await this.client.controller.read.rootGeneration([BigInt(this.rootId)], {blockNumber:setup.source.blockNumber}) : null;
      const recorded = operationKey ? await this.client.controller.read.getOperation([BigInt(this.rootId),BigInt(this.rootId),generation,operationKey], {blockNumber:setup.source.blockNumber}) : null;

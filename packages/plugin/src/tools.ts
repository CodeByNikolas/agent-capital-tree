...
export const toolSpecs = {
  ...
  openPosition: { description: 'Open the configured Uniswap position within mandate limits.', schema: z.object({ nodeId: id, maxAmount0: amount, maxAmount1: amount, minLiquidity: amount, deadline }).strict(), readOnly: false },
  increasePosition: { description: 'Increase the configured position within mandate limits.', schema: z.object({ nodeId: id, maxAmount0: amount, maxAmount1: amount, minLiquidity: amount, deadline }).strict(), readOnly: false },
  collectFees: { description: 'Collect earned fees to the bound vault.', schema: z.object({ nodeId: id, deadline }).strict(), readOnly: false },

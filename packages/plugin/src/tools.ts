...
export const toolSpecs = {
  ...
  createChildVault: { description: 'Preferred chat demo: atomically create a named child ENS/vault and allocate bounded capital, without Docker, CLIProxyAPI or a background AI worker. The current chat remains the decision maker. Only the local onchain-authorized operator signs. Reuse the same operationKey and identical arguments on retries; read getTree afterwards. This does NOT launch autonomous inference.', schema: z.object({ operationKey, name: z.string().regex(/^[a-z][a-z0-9-]{0,30}$/), asset: address, amount, restrictions }).strict(), readOnly: false },

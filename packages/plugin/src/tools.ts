...
export const toolSpecs = {
  ...
  spawnChild: { description: 'Create a funded child capital worker with its own on-chain vault, ENS mandate and isolated runtime. When a subagent needs allocated money, use this MCP tool; native Codex subagents do not create vaults or transfer capital. Choose an explicitly authorized asset, raw-unit amount and narrower capabilities, allowed assets, per-action limits and expiry. There is no default allocation: 10 Sepolia test USDC is 10000000 raw units (six decimals), not real dollars. The parent needs delegate authority and free balance; a child making on-chain swaps also needs separately funded Sepolia ETH for gas. Reuse the same operationKey and exact arguments after an uncertain result; reconcile with getOperationStatus and current chain state before trying a new operation.', schema: z.object({ operationKey: operationKey.describe('Fresh 32-byte idempotency key for this exact spawn; retain it and all arguments for uncertain-result reconciliation.'), name: z.string().regex(/^[a-z][a-z0-9-]{0,30}$/).optional().describe("Readable ENS label, e.g. researcher; unique under its parent. Reuse the same name on retries."), task: z.string().min(1).max(12000).describe('Child work to perform after confirmed allocation. Task text does not grant financial rights.'), model: id, asset: address.describe('ERC-20 address of the explicitly authorized allocation asset; use official Sepolia test USDC only when requested.'), amount: amount.describe('Explicit allocation in raw token units, with no assumed default. Official Sepolia test USDC has six decimals: 10 USDC = 10000000.'), restrictions: restrictions.describe('Child mandate narrowed from the parent. Specify capabilities, allowedAssets, maxPerAction in raw units, and expiresAt for the intended work; PAY and swap are separate rights.') }).strict(), readOnly: false },

...
export function prepareRenderer() {
  return ready ??= (async () => {
    ...
    const fonts = await Promise.all(['IBMPlexSans.ttf', 'Fraunces.ttf', 'IBMPlexMono-Regular.ttf'].map(name =>
...
export async function svgAsPng(svg) {
  ...
  const renderer = new Resvg(svg, { font: { fontBuffers, defaultFontFamily: 'IBM Plex Sans' } });
  let rendered;

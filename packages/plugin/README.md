...
For a **real read-only Codex or Claude chat**, use the separate `scripts/mcp-readonly-server.mjs`. It registers only `getTree` and `visualizeTree` (live PNG plus Mermaid fallback), reads the public USDC controller directly through the SDK, and needs no runtime token, signer or Docker. `pnpm mcp:chat-verify` exercises that STDIO server; `pnpm mcp:settings` prints host-setting paths. The exact registration and sample prompts are in [the local setup guide](../../docs/local-setup.md). Do not mistake these two read tools for the full companion's 16 tools, and do not register both servers in one Codex profile if duplicate `getTree` names would be confusing. The public dashboard independently reads Sepolia and cannot observe your local Codex/Claude session.

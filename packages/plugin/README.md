...
For a **real read-only Codex chat** in the ChatGPT desktop app, use the separate `scripts/mcp-readonly-server.mjs`. It registers only `getTree`, reads the public USDC controller directly through the SDK, and needs no runtime token, signer or Docker. `pnpm mcp:chat-verify` exercises that STDIO server; the exact PowerShell/Bash registration and sample prompt are in [the local setup guide](../../docs/local-setup.md). Do not mistake its one tool for the full companion's 16 tools, and do not register both servers in one profile if duplicate `getTree` names would be confusing. The public dashboard independently reads Sepolia and cannot observe your local Codex session.

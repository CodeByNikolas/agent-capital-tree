...
For a fresh **marketplace installation** on Codex CLI 0.154.0, use `codex plugin marketplace add /absolute/path/to/this/repo` followed by `codex plugin add agent-capital-tree@agent-capital-tree`. The CLI command is `plugin add` in this version; `plugin install` is not available. `codex mcp list --json` should then show the enabled `capital-tree` server from the installed plugin cache. Set `ACT_RUNTIME_URL` and `ACT_MCP_TOKEN` in the launching shell as above before using its tools. Do not add the explicit `capital_tree_root` MCP registration to the same profile.

The read-only installation check is `node scripts/test-plugin-install.mjs /absolute/private-runtime-config.json` after building the SDK. It makes a fresh temporary Codex profile, installs from this repository's marketplace, starts the **installed** MCP server, and calls `getTree` against the deployed Sepolia controller through a local read-only bridge. The bridge exposes no write endpoint and does not start a model or operator companion. A completed root can therefore be inspected without reactivating its revoked runtime. This proves installation, server registration, MCP handshake and live chain read; it does not prove marketplace-installed financial writes.
